...
def cha_seconds(chuo_da,chuo_xiao):
    ...
    :return:
    """
    return (datetime.datetime.fromtimestamp(chuo_da) - datetime.datetime.fromtimestamp(chuo_xiao)).seconds


def get_guoguo_count(tie):
    response = requests.get(tie.url, cookies=zk_cks)
    content_decode = response.content.decode('gbk')
    return content_decode.split('人评分, 查看全部评分')[0][-1]
...
def zhuan_ba_tie_hot():
    ...
    for tie in ties_in_one_hour:
        ...
        if 300 <= seconds < 360 and tie.wu_guo is None:
            ...
            #todo 将果塞入db里
            # record = db.session.query(Tie).filter_by(thirdorder=thirdorder).first()
            # record.reason = reason
            db.session.commit()
...
def zhuanba_task():
    ...
    :return:
    """
    response = requests.get('http://www.zuanke8.com/forum-15-1.html', cookies=zk_cks)
    content_decode = response.content.decode('gbk')
    ...
    final_add_ties = []
    for tie in ties_in_one_hour:
        db_tie_urls.append(tie.url)
    for new_tie in all_new_ties:
        if new_tie['url'] not in db_tie_urls:
...
def tb_order_task():
    ...
    scheduler.add_job(func=zhuanba_task, trigger='interval', seconds=60, id='zhuanba_new_tie_task')
    scheduler.start()
